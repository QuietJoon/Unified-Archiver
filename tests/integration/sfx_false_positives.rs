...
fn test_executable_with_fake_signature_at_end_not_sfx() {
    // Executable with archive signature too close to EOF (< 100 bytes remaining)
    let mut temp = NamedTempFile::new().unwrap();

    let stub = b"#!/bin/sh\necho test\n";
    let padding = vec![0u8; 400];
    let zip_sig = b"PK\x03\x04";
    // Only 50 bytes after signature (< 100, should be rejected)
    ...
    let result = Archive::detect_sfx(temp.path()).unwrap();
    // Should reject due to insufficient data after signature
    assert!(
        !result.is_sfx,
