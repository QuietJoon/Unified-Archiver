...
///
/// ## Why the fixture is committed rather than built (OI-0056-010)
///
/// The lane needs a `tar` that actually sparse-**encodes** a member. bsdtar —
/// the `tar` on macOS, this project's primary dev platform — accepts `-cSf`
/// and exits 0 but documents `-S` as extract-mode-only, so it writes a dense
/// ~1 MiB archive and the hole-materialisation tripwire cannot fire.
///
/// Driving `tar` from here therefore had two bad outcomes and no good one: it
/// used to `eprintln!` and return on such a host, reporting success having
/// proved nothing on every default run of the primary dev platform, and the
/// `#[ignore]` that replaced the false green traded a silent non-test for a
/// visible one. Neither exercised the risk DCR-011 declares.
///
/// `tests/fixtures/sparse.tar` is generated once by
/// `scripts/generate-tar-fixtures.sh` on a GNU-tar host and checked in, so the
/// lane is a plain `#[test]` that runs everywhere — no host `tar`, no skip, no
/// `#[ignore]`. Waiting for CI on a GNU-tar host was the recorded plan;
/// committing the bytes is cheaper, needs no CI, and removes the host
/// dependency permanently — the same trade `scripts/generate-rar-fixtures.sh`
/// already makes for RAR.
///
