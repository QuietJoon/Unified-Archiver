...
description: "Design baseline ID: BL-001-retroactive"
tags: [project-control, ADR-0019, ADR-0021, ADR-0020, ADR-0040, DCR-001, OI-0057-003]
...
- Summary: All five user stories implemented. DCR-001 closed Phases A+B on 2026-04-13:
  - **OI-027-001 RESOLVED** (Phase A.1): unknown-stub SFX scanning proceeded to the signature scan instead of bailing at Stage 1. SCN-SFX-08 → Covered. **Superseded 2026-08-16 (R0070-0076):** the decision was reversed — an `Unknown` stub is now rejected at Stage 1 and `detect_sfx` returns `not_sfx()` before the scan. SCN-SFX-08 is now the *rejection* scenario and remains Covered; see `docs/architecture/scenario-matrix.md` and `docs/records/AD-0060-r0070-broad-modular-triage-closure.md`.
  ...
  - **OI-025-002 RESOLVED** (2026-04-14): `commit_changes()` preserves modification time and Unix permissions via `add_file_from_data_with_metadata()` on both ZipWriter and LibarchiveArchive backends. `preserve_metadata` flag is now functional.
  - **DEF-006 Closed**, **DEF-007 Closed**, **DEF-008 Closed** (2026-04-14) in stub manifest.

  Remaining tracked gaps:
  - DEF-002 (split creation), DEF-004 (true streaming for non-libarchive backends), DEF-005 (some ZIP modify edge cases) — out-of-MVP-scope and tracked as DEFERRED. DEF-001 (`open_at_offset`) closed 2026-04-18 (tempfile-backed implementation; see AD 0040). DEF-003 (SecStr migration) closed 2026-04-17 (OI-0057-003).
...
- Design change records: `docs/records/DCR-001-in-flight-gap-remediation.md`
- New ADRs: `docs/records/AD-0019-unrar-process-wide-mutex.md`, `docs/records/AD-0020-additive-modification-options-extension.md`, `docs/records/AD-0021-per-entry-creation-progress.md`
