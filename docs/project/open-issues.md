...
- **Status:** PARTIALLY LANDED (2/6 as of 2026-06-06) — the two self-contained, no-dependency durability/correctness items landed: **item 3 (R0076-0017)** — `ZipWriter::finish` now captures the `File` from `RawZipWriter::finish()`, `sync_data()`s it, and fsyncs the parent dir via `super::common::sync_parent_dir`, giving ZIP creation the same crash-consistency as extract/modify (idempotent with the `Drop`-finalize via `writer.take()`); **item 4 (R0076-0035)** — `extract_file_with_options` now stages each entry to a sibling tempfile (`build_staging_path`) and `rename_with_overwrite`s on success, with the staging file removed on every error path, so a `copy_data`/header/finish failure can no longer leave a partial file at the destination (mirrors extract-all, including the `!overwrite` pre-check and `sanitize_entry_path` traversal guard). Both adversarially verified (handle-cleanup / partial-file-guarantee / drop-idempotency — pass) and covered by the green `--all-features` suite (extraction, creation, integrity, zip/7z binaries). **Items 1 and 2 landed 2026-09-03** (`8517e56`, `8d86fa5`; ticgit `5ebf46`). Item 1 (R0076-0005): `security::create_parent_dirs_verified` pairs `create_dir_all` with a re-canonicalisation of the created parent against the destination, wired into all eight extraction sites across the four backends; the two UnRAR sites also lost a `!parent.exists()` guard that would have skipped the check in exactly the case a swap produces. It narrows the TOCTOU window rather than closing it — a swap between the verification and the write still wins, which needs `openat`-relative writes. Its recorded coupling to "the AD 0066 v0.4 strict-path flag" had already dissolved: `reject_unsafe_paths` exists and is enforced. Item 2 (R0076-0014): both writers now reach the filesystem through `open_file_no_follow_symlinks`, and its `#[expect(dead_code)]` is gone; the "needs a Unix `libc` dep" blocker covered only the evolution to a native `O_NOFOLLOW`, not the call-site wiring. **Still OPEN:** items 5 (R0076-0045 staging unlink→reopen-by-name race — needs owned-fd / libarchive callback design, MADR-0009), 6 (R0076-0089 `commit_changes` phase split — couples to AD 0053 D2 `ModifyArchive`).
...
- [ ] Gate green on the typical backend matrix  <!-- Reworded 2026-09-03 (AD-0070). This read "CI green ..." and was marked "cannot be satisfied ... the repository has no CI configuration at all". AD-0070 retires that: CI here means a committed recipe run on an observing host with the native exit status recorded against a fingerprint, and `scripts/release-gate.sh` is the recipe. The backend matrix is observable on the dev host, so this box is now decidable rather than blocked -- it needs a recorded run under docs/verification/, not a service. Only the Windows arm needs a different machine (OI-0065-001). -->
...
- Host-dependent skips: **restated 2026-09-03 (AD-0070).** This read "CI absence: the repository
  still has no CI configuration (OI-0065-001), so nothing catches a host-dependent skip today."
  No hosted CI is coming, so worded that way the concern could never be discharged. The durable
  answer is not a service but the pattern this repository has now used twice — commit the fixture
  and delete the host dependency (`scripts/generate-rar-fixtures.sh`, and
  `scripts/generate-tar-fixtures.sh` with `tests/fixtures/sparse.tar` on 2026-09-03). A lane that
  needs no host tool cannot skip on a host, so there is nothing left for CI to catch.
