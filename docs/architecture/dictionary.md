...
| ArchiveError | Unified error enum with variants: Io, Format, Corruption, Password, Unsupported, CodecUnavailable, UnsupportedOperation, InvalidPath. Note: `Unsupported` is returned for format-level inability (e.g., `open_at_offset()`); `UnsupportedOperation` is returned for mode-level restrictions (e.g., writing to a read-only handle). | N/A | `src/error.rs` |
