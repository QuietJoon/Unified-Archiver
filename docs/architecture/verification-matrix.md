...
| SCN-CRE-03 | Create password-protected archive (ZIP-only) | No | No | Integration | `tests/integration/creation.rs` | Partial — tests cover ZIP encryption only; 7z/TAR encrypted creation is not implemented |
...
| Concurrency tests | `tests/integration/concurrency.rs` | Thread safety, parallel operations. Concurrent RAR access serialized via process-wide `UNRAR_LOCK` mutex (OI-026-004 resolved 2026-04-13); `test_concurrent_rar_open_and_list` runs 8 threads × 20 iterations against RAR4 + RAR5 fixtures. |
