...
Found in Review 027 (Issues R027-001, R027-004, R027-005).
...
## Decision Outcome
ACCEPT: Option 1 — rename to `ScriptInterpreter`, add `Unknown` variant. Detection behavior unchanged (Unknown exists for future use; current detection still returns `not_sfx()` for unrecognized stubs). Scanning through Unknown stubs tracked as OI-027-001.
...
* Neutral: detection behavior unchanged for now (OI-027-001 tracks the extension)
